...
taxonomy.results.reduced <- taxonomy.results %>% select(asvHash,isFish,kingdom,phylum,class,order,family,sintaxSpeciesID,isContaminated,nucleotides)

# remove non-assigned and collapse asvs
fishes.by.sample <- asvs.by.sample %>% left_join(taxonomy.results.reduced,by="asvHash") %>% 
    filter(isContaminated==FALSE & !is.na(isFish)) %>%
    group_by(primerSet,library,eventID,sampleHash,replicateFilter,kingdom,phylum,class,order,family,sintaxSpeciesID,asvHash,nucleotides) %>% 
    summarise(nreads=sum(nreads),.groups="drop") %>%
    arrange(primerSet,library,eventID,sampleHash,desc(nreads)) %>%
...
# write out fishes after removing blanks and MBA samples
fishes.by.sample %>% 
    filter(!grepl("Blank",replicateFilter)) %>% 
    select(-replicateFilter) %>%
    filter(grepl("EA-",eventID)) %>% 
    write_csv(file=here("results/results-by-sample.csv"))
...
events.df.blanks <- events.df %>% 
    filter(grepl("Blank",replicateFilter)) %>% 
    mutate(sintaxSpeciesID=NA,nReads=0) %>% 
    filter(!sampleHash %in% pull(blanks.with.fishes,sampleHash))

# combine and sort (and collapse)
blanks.all <- bind_rows(blanks.with.fishes,events.df.blanks) %>% 
    select(primerSet,library,replicateFilter,eventID,sampleHash,sintaxSpeciesID,nReads) %>%
    rename(blankType=replicateFilter) %>%
    group_by(primerSet,library,blankType,eventID,sampleHash,sintaxSpeciesID) %>% 
    summarise(nReads=sum(nReads),.groups="drop") %>%
    arrange(primerSet,library,blankType,eventID,sampleHash,desc(nReads)) %>%
    filter(!grepl("MBA-",eventID))
...
# report
writeLines("\n...\nReads by sample written to: 'results/results-by-sample.csv'\n")
